...
export default function MemberResetPasswordForm({ token }: { token: string }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!token) {
      setError(t("auth.invalidResetToken", "Reset token is missing or invalid."));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || t("auth.resetFailed", "Unable to reset password"));
        setLoading(false);
        return;
      }

      setMessage(data.message || t("auth.resetSuccess", "Password updated successfully."));
      setLoading(false);
      setTimeout(() => {
    ...
    <div className="min-h-[calc(100vh-56px)] flex items-center justify-center px-6 py-20">
      <div className="w-full max-w-[420px] rounded-2xl border border-[var(--border)] bg-white p-7 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-[var(--text-light)]">
          {t("auth.resetPasswordHint", "Enter your new password below.")}
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-[12px] font-semibold uppercase tracking-wider text-[var(--text-light)]">{t("auth.newPassword", "New password")}</label>
            <input
              type="password"
              value={password}
          ...
          <div>
            <label className="mb-1.5 block text-[12px] font-semibold uppercase tracking-wider text-[var(--text-light)]">{t("auth.confirmPassword", "Confirm password")}</label>
            <input
              type="password"
          ...
          >
            {loading ? t("auth.updatingPassword", "Updating password...") : t("auth.updatePassword", "Update password")}
          </button>
        </form>
