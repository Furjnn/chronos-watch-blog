    ...
    <section className="py-16 md:py-[72px] bg-[var(--bg-warm)]">
      <div className="max-w-[1200px] mx-auto px-6 md:px-10">
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <span className="text-[11px] font-semibold tracking-[2.5px] uppercase text-[var(--gold)]">{t("home.popular.badge", "Trending Now")}</span>
        </div>
        <h2 className="text-[34px] font-medium text-[var(--charcoal)] leading-tight mb-1.5" style={{ fontFamily: "var(--font-display)" }}>{t("home.popular.title", "Most Popular This Week")}</h2>
        <div className="w-10 h-0.5 bg-[var(--gold)] mb-9" />

        <div className="grid grid-cols-1 lg:grid-cols-[1.35fr_1fr] gap-6">
          <Link
            href={localizePath(`/blog/${featured.slug}`)}
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-7">
              ...
              <div className="flex items-center gap-3.5 text-[12px] text-white/50">
                ...
                <span className="flex items-center gap-1">👁 {featured.views} {t("home.popular.viewsSuffix", "views")}</span>
              </div>
            </div>
          </Link>
