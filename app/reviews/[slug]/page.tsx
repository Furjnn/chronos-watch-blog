...
import { reviewBySlugQuery, allReviewSlugsQuery } from "@/sanity/queries";
...
export async function generateStaticParams() {
  const slugs = await client.fetch(allReviewSlugsQuery);
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: `${review.title} Review`, description: review.verdict };
}

function formatImage(img: any, w = 800) {
...
export default async function ReviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const gallery = (review.gallery || []).map((img: any) => formatImage(img));
        ...
        <div className="max-w-[1200px] mx-auto px-6 md:px-10 pt-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pb-12 border-b border-[var(--border)]">
            ...
            <div>
              ...
              <p className="text-[14px] text-[var(--text-secondary)] mb-5">Ref. {review.watchRef}</p>
              ...
              {review.priceRange && (
                <div className="mb-5">
                  ...
                  <div className="text-[28px] font-medium text-[var(--charcoal)]" style={{ fontFamily: "var(--font-display)" }}>
                    ${review.priceRange.min?.toLocaleString()} — ${review.priceRange.max?.toLocaleString()}
                  </div>
                </div>
              )}
      ...
      {review.verdict && (
        <section className="bg-[var(--bg-warm)] py-9 border-b border-[rgba(184,149,106,0.15)]">
          <div className="max-w-[1200px] mx-auto px-6 md:px-10">
            ...
            <span className="text-[12px] text-[var(--text-light)] mt-2 block">— {review.author?.name}, {review.author?.role}</span>
          </div>
        </section>
      )}
        ...
        <section className="py-12 bg-[var(--bg)]">
          <div className="max-w-[800px] mx-auto px-6 md:px-10 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="p-7 border border-[rgba(45,106,79,0.2)] bg-[rgba(45,106,79,0.02)]">
              ...
              {(review.prosAndCons.pros || []).map((p: string) => (
            ...
            <div className="p-7 border border-[rgba(185,28,28,0.15)] bg-[rgba(185,28,28,0.02)]">
              ...
              {(review.prosAndCons.cons || []).map((c: string) => (
